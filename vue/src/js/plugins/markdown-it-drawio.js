module.exports = (md, opt) => {
    ...
    function validate(params, /*markup*/) {
        return params.trim().match(/^drawio$/);
    }
    ...
    function parser(state, startLine, endLine, silent) {
        ...
        let token = state.push('drawio', 'div', 0);
        token.markup = markup;
    ...
    md.block.ruler.before('fence', 'drawio', parser, {
        alt: ['paragraph', 'reference', 'blockquote']
    });
    /*
    md.renderer.rules['drawio_open'] = (token, idx) => {
        console.log(token, idx);
    }
    */
    md.renderer.rules['drawio'] = (token, idx) => {
